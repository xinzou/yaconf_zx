...
//合并配置文件目录
void merge_ini_directory(int *ndir, struct dirent **namelist[],const char *dirname, char *tmp_path){
	uint32_t j;
	int ndir_1;
	char *p;
	ndir_1 = *ndir;
	char child_path[MAXPATHLEN];
	struct dirent **tmp_namelist;
	snprintf(child_path, MAXPATHLEN, "%s%c%s", dirname, DEFAULT_SLASH, tmp_path);
	int l;
	int ndir_2;
	if ((ndir_2 = php_scandir(child_path, &tmp_namelist, 0, php_alphasort)) > 0) {
		for (l = 0; l < ndir_2; l++)
		{
			if(!strcmp(tmp_namelist[l]->d_name, ".") || !strcmp(tmp_namelist[l]->d_name, "..")){
				continue;
			}
			if (!(p = strrchr(tmp_namelist[l]->d_name, '.')) || strcmp(p, ".ini")) {
				char tmp_path_t[MAXPATHLEN];
				snprintf(tmp_path_t, MAXPATHLEN, "%s%c%s", tmp_path, DEFAULT_SLASH, tmp_namelist[l]->d_name);
				merge_ini_directory(ndir, namelist, dirname, tmp_path_t);
				continue;
			}
			char tmp_file_path_t[MAXPATHLEN];
			snprintf(tmp_file_path_t, MAXPATHLEN, "%s%c%s", tmp_path, DEFAULT_SLASH, tmp_namelist[l]->d_name);
			strcpy(tmp_namelist[l]->d_name, tmp_file_path_t);
			(*namelist)[(*ndir)++] = tmp_namelist[l];
...
PHP_MINIT_FUNCTION(yaconf)
{
	...
	if ((dirname = YACONF_G(directory)) && (dirlen = strlen(dirname)) 
		...
		if ((ndir = php_scandir(dirname, &namelist, 0, php_alphasort)) > 0) {
			...
			//递归读取目录配置文件
			for (t = 0; t < ndir_t; t++) {
				if(!strcmp(namelist[t]->d_name, ".") || !strcmp(namelist[t]->d_name, "..")){
					continue;
				}
				if (!(p = strrchr(namelist[t]->d_name, '.')) || strcmp(p, ".ini")) {
					snprintf(&tmp_path, MAXPATHLEN, "%s", namelist[t]->d_name);
					merge_ini_directory(&ndir, &namelist, dirname, tmp_path);
				}
			}
